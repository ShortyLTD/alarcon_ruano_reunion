...
4. Branded domain: santacruzreunion.com (GoDaddy DNS) now points at the kit; www and the *.vercel.app hosts redirect to it. The original guest website is still on its legacy Netlify URL pending migration. Restore its complete source and assets into a separate GitHub repository and Vercel project, attach `2026.santacruzreunion.com` in Vercel, verify the guest experience, then update `NEXT_PUBLIC_ORIGINAL_SITE_URL`. Retire the old host only after the Vercel replacement is verified. Do not add new domains or hosting to Netlify.
