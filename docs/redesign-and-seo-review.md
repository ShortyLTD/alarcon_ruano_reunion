...
4. Branded domain: resolved. Vercel DNS serves santacruzreunion.com; GoDaddy is registrar-only. The Git-connected `da-boyz/alarcon_ruano_reunion` project serves the kit, with www and legacy kit hosts redirecting to the apex. The complete original guest guide is preserved in `ShortyLTD/santacruzreunion-2026` and deployed by `da-boyz/reunion-2026-guest-site` at https://2026.santacruzreunion.com/. HTTPS, the map, filters, all 16 lodging suggestions and section anchors passed desktop and mobile verification. `NEXT_PUBLIC_ORIGINAL_SITE_URL` defaults to that address; the guest subdomain is excluded from kit redirects.
