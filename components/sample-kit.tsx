...
export default function SampleKit() {
  ...
  return <div className="mk-sample">
    <div className="mk-sample-top"><div><span className="mk-label">THE RIVERA FAMILY · EXAMPLE</span><h3>A weekend for 60, coming together.</h3><p>Sample details. Dates flexible. Places still to confirm.</p></div><button className="mk-button mk-button-light" onClick={() => { try { exportPacket(sample, vendors); setStatus('Your sample ZIP is downloading.'); } catch { setStatus('The download did not start. Please try again.'); } }}><Download size={16}/> Download sample</button></div>
    <div className="mk-sample-nav" aria-label="Sample sections">{[{id:'places',label:'Your local shortlist',icon:MapPin},{id:'inquiry',label:'Your hotel inquiry',icon:Mail},{id:'guests',label:'Your guest guide',icon:Users}].map(item => <button key={item.id} aria-pressed={view===item.id} onClick={() => setView(item.id)}><item.icon size={16}/>{item.label}</button>)}</div>
    ...
    </div><p className="mk-sample-foot">Your download includes editable inquiries, a printable plan, contacts, budget sheets, and a guest guide.<span role="status">{status}</span></p>
  </div>;
}
