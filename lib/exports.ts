import { dateLabel, inquiry, money, suggestTasks } from './planner';
...
export function guestGuideHtml(workspace: Workspace, vendors: Vendor[]): string {
  const b = workspace.brief;
  const name = b.family.trim() || 'Our family';
  const email = emailAddress(b.email.trim());
  const mailto = email ? `mailto:${email}?subject=${encodeURIComponent(`${name} reunion RSVP`)}&body=${encodeURIComponent('Household name:\nWho is attending:\nWhich activities / meals:\nQuestions for the organizer:\n')}` : '';
  const rsvp = mailto ? `<p><a href="${escapeHtml(mailto)}">Email your RSVP to ${escapeHtml(b.organizer || 'the organizer')}</a></p><p class="muted">This opens your email app. Review and send the message yourself. This guide does not collect or track RSVPs.</p>` : '<p>Contact the reunion organizer to RSVP. An organizer email has not been added to this guide.</p>';
  return layout(`${name} · Santa Cruz reunion`, `<p class="muted">SANTA CRUZ REUNION KIT · GUEST GUIDE</p><h1>${escapeHtml(name)}. Together in Santa Cruz.</h1><p>${escapeHtml(dateLabel(b.date))} · ${escapeHtml(b.nights)} nights</p><div class="note">Planning in progress. Confirm arrangements with your organizer before making travel bookings. This is a downloadable snapshot, not a live website.</div><p style="white-space:pre-wrap">${escapeHtml(workspace.guestMessage || 'We can’t wait to spend a little time together by the coast. Here is the plan so far.')}</p><h2>Our weekend</h2>${scheduleHtml(workspace)}<h2>Places we’re considering</h2>${vendorHtml(vendors.filter(v => workspace.selected.includes(v.id)))}<h2>Getting here</h2><p>${link('https://www.google.com/maps/search/?api=1&query=Santa+Cruz+California', 'Open Santa Cruz in maps')}. Confirm the exact gathering address, parking, and accessible arrival route with your organizer.</p><h2>What to bring</h2><p>Layers for cooler mornings and evenings, comfortable shoes, sun protection, a water bottle, and anything you need for a comfortable day together.</p><h2>Let us know you’re coming</h2>${rsvp}<p class="muted">Prepared ${escapeHtml(new Date().toLocaleDateString('en-US'))}. Ask your organizer for the latest version.</p>`);
}

/** A portable HTML preview; downloading does not publish or host a website. */
export function invitationText(workspace: Workspace): string {
  const b = workspace.brief;
  return `${b.family || 'Our family'} reunion — Santa Cruz\n\n${dateLabel(b.date)} · ${b.nights} nights\n\n${workspace.guestMessage || 'Let’s get everyone together for a little time by the coast. We’re putting together our Santa Cruz reunion and would love you to join us.'}\n\nThe attached guest guide contains our proposed schedule and places we’re considering. Arrangements are still being confirmed; please check with us before booking travel.\n\nTo RSVP, reply with your household name, who is coming, and which meals and activities you expect to join.\n\n${b.organizer || 'Your reunion organizer'}${b.email ? `\n${b.email}` : ''}\n`;
}
...
export function createPacket(workspace: Workspace, vendors: Vendor[]): Blob {
  ...
  const rows: unknown[][] = [['Provider', 'Status / evidence', 'Total amount USD', 'Deposit USD (part of total; not an extra expense)', 'Deadline', 'Notes']];
